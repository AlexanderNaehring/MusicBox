...
// Otherwise it connects to WiFi, downloads it from DOWNLOAD_BASE_URL + path into a
// cache folder, verifies every downloaded file's size before moving anything into
// its final place, plays audio cues to keep the user informed, and disconnects WiFi
// again before returning. A download that is interrupted or fails verification never
// touches the final location - `path` is only created once everything is confirmed.
//
// `path` is treated as a single mp3 file if it ends in ".mp3", otherwise as a folder.
// Folder downloads require a manifest at <DOWNLOAD_BASE_URL><path>/manifest.json:
